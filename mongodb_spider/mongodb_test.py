...
def insert_one_doc(db):
    ...
    print('-----------插入一个document', information_id)

def get_one_doc(db):
  # 有就返回一个，没有就返回None
  coll = db['informations']
  print("返回第一条记录: ", coll.find_one())  # 返回第一条记录
  print('----------quyang', coll.find_one({"name": "quyang"}))
  print('--------none', coll.find_one({"name": "none"}))
...
def get_one_by_id(db):
  ...
  print('----------obj_id', coll.find_one({"_id": obj_id}))
